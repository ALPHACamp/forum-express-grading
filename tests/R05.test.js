  ...
  context('# [網址正確、畫面正常執行]', () => {
    before(async () => {
      ...
      this.getUser = sinon
        ...
        .returns({ id: 1, Followings: [], FavoriteRestaurants: [] })
    })
  ...
  context(
    ...
    () => {
      before(async () => {
        ...
        this.favoriteMock = createModelMock('Favorite', [{userId: 1, restaurantId: 1}], 'FavoriteUsers', mockRestaurantData)
      ...
      it(' POST favorite ', async () => {
        ...
        res.render.getCall(0).args[1].restaurants[1].favoriteCount.should.equal(1)
      })
