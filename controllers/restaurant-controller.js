...
const restaurantController = {
  ...
  getRestaurant: (req, res, next) => {
    return Restaurant.findByPk(req.params.id, {
      ...
      .then(restaurant => {
        const isLiked = restaurant.dataValues.LikedUsers.some(l => l.id === req.user.id)
        const isFavorited = restaurant.dataValues.FavoritedUsers.some(f => f.id === req.user.id)
        restaurant.update({
          view_counts: (restaurant.dataValues.view_counts + 1)
        })
