    ...
    try {
      ...
      const datas = restaurants
        .map(r => ({
          ...
          isFavorited: r.FavoritedUsers.some(fu => fu.id === req.user?.id)
        }))
        .sort((a, b) => b.favoritedCount - a.favoritedCount)
        .slice(0, limitOfRestaurants)
      res.render('top-restaurants', { restaurants: datas })
    } catch (err) {
      next(err)
    }
  }
