...
const userController = {
  ...
  signUp: (req, res, next) => {
    ...
    User.findOne({ where: { email: req.body.email } })
      .then(user => {
        ...
        return bcrypt.hash(req.body.password, 10) // 前面加 return,讓這個 Promise resolve 的值可以傳到下一個 .then 裡面
      })
  ...
  removeLike: (req, res, next) => {
    return Like.findOne({
      ...
      .then(like => {
        if (!like) throw new Error("You haven't likeed this restaurant")
  ...
  getTopUsers: (req, res, next) => {
    ...
    return User.findAll({
      ...
      .then(users => {
        ...
        users = users.sort((a, b) => b.followerCount - a.followerCount) // 把追蹤者多的人排在前面
